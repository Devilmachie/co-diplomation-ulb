...
;
(function ($) {
  // Get the value of the cookie warning-global-bypass.
  // https://developer.mozilla.org/en-US/docs/Web/API/document/cookie
  var warning_global_bypass = document.cookie.replace(/(?:(?:^|.*;\s*)warning-global-bypass\s*\=\s*([^;]*).*$)|^.*$/, "$1");

  // Get the global warning creation time.
  var warning_global_timestamp = Drupal.settings.warning_global.timestamp;

  // Display the global warning only if the cookie is older than the creation time.
  if (warning_global_bypass > warning_global_timestamp) {
    $("#warning-global").remove();
  }

  // Hide the global warning when and set the cookie when we click on the X.
  $("#warning-global-close").click(function() {
    document.cookie = "warning-global-bypass=" + Math.floor(Date.now() / 1000) + "; path=/";
    $("#warning-global").remove();
  });
})(jQuery);
;
